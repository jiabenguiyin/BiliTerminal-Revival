...
public class AndroidMediaPlayer extends AbstractMediaPlayer {
    ...
    private void attachInternalListeners() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            TimedTextListenerApi16.attach(mInternalMediaPlayer, this);
        }
    }
    ...
    private static final class TimedTextListenerApi16 {
        private static void attach(MediaPlayer player, AndroidMediaPlayer owner) {
            final WeakReference<AndroidMediaPlayer> weakOwner = new WeakReference<>(owner);
            player.setOnTimedTextListener((mp, text) -> {
                AndroidMediaPlayer mediaPlayer = weakOwner.get();
                if (mediaPlayer == null) {
                    return;
                }
                ...
                mediaPlayer.notifyOnTimedText(ijkText);
            });
        }
    }
}
