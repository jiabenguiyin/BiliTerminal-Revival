...
public class OpusContentAdapter extends RecyclerView.Adapter<OpusContentAdapter.ArticleLineHolder> {
    ...
    public OpusContentAdapter(Activity context, Opus article) {
        ...
        if (article.upInfo == null) article.upInfo = new com.RobinNotBad.BiliClient.model.UserInfo();
        if (article.stats == null) article.stats = new com.RobinNotBad.BiliClient.model.Stats();
        this.paragraphs = article.paragraphs != null ? article.paragraphs : new OpusParagraph[0];
    }
    ...
    public void onBindViewHolder(@NonNull ArticleLineHolder holder, int position) {
        ...
        if (viewType >= 0 && (realPosition < 0 || realPosition >= paragraphs.length))
            return;
        ...
        switch (viewType) {
            ...
            case -2:
                ...
                String contentId = (article.id > 100000000L ? "opus" : "cv") + article.id;
                cvidText.setText(contentId);
                StringUtil.setCopy(cvidText, contentId);
                if (article.stats.view > 0) {
                    viewCount.setVisibility(View.VISIBLE);
                    holder.itemView.findViewById(R.id.viewIcon).setVisibility(View.VISIBLE);
                } else {
                ...
                if (!TextUtils.isEmpty(article.pubTime)) {
                    ...
                    holder.itemView.findViewById(R.id.timeIcon).setVisibility(View.VISIBLE);
                } else {
                    timeText.setVisibility(View.GONE);
                    holder.itemView.findViewById(R.id.timeIcon).setVisibility(View.GONE);
                }
