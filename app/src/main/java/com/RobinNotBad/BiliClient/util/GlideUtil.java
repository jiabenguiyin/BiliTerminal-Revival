...
public class GlideUtil {
    ...
    public static String url(String url) {
        ...
        String result;
        boolean legacyJpeg = Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR2;
        if (legacyJpeg || SharedPreferencesUtil.getBoolean("image_request_jpg", false)) {
    ...
    public static String url_hq(String url) {
        ...
        if (legacyJpeg || SharedPreferencesUtil.getBoolean("image_request_jpg", false)) {
            if (!legacyJpeg && (url.endsWith("jpeg") || url.endsWith("jpg")))
                return NetWorkUtil.routeUrlForRelay(url);
            int maxWidth = legacyJpeg ? 768 : MAX_W_HIGH;
            result = url + "@0e_"
                    + QUALITY_HIGH + "q_"
                    //+ MAX_H_HIGH + "h_"
                    + maxWidth + "w.jpeg";
    ...
    public static TransitionOptions<?, ? super Drawable> getTransitionOptions() {
        boolean defaultTransition = Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP;
        if (SharedPreferencesUtil.getBoolean(SharedPreferencesUtil.LOAD_TRANSITION, defaultTransition)) {
