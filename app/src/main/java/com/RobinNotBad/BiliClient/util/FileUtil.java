...
public class FileUtil {
    ...
    public static boolean checkStoragePermission() {
        if (VideoStorageUtil.isSafMode()) return true;
    ...
    public static List<StorageLocation> getVideoStorageLocations(Context context) {
        ...
        for (int i = 0; i < externalDirs.length; i++) {
            File base = externalDirs[i];
            if (base == null) continue;
            File videoDir = new File(base, "video");
            boolean removable = i > 0 || (Build.VERSION.SDK_INT >= 21 && Environment.isExternalStorageRemovable(base));
            String label = removable ? "外置 SD 卡 " + removableIndex++ : "设备存储";
            locations.add(new StorageLocation(label, videoDir, removable));
        }
        return locations;
    }

    public static boolean prepareVideoStorageLocation(File path) {
        if (path == null) return false;
        if (!path.exists() && !path.mkdirs()) return false;
        if (!path.isDirectory() || !path.canWrite()) return false;
        try {
    ...
    public static final class StorageLocation {
        ...
        public StorageLocation(String label, File path, boolean removable) {
            this.label = label;
            this.path = path;
    ...
    public static void requireTFCardPermission() {
        // Removable-card root access must be requested with ACTION_OPEN_DOCUMENT_TREE.
        // The Activity owns the result callback; see SettingLaboratoryActivity.
    }
