...
public class ConfInfoApi {
    ...
    public static String signWBI(String url_query) throws JSONException, IOException {
        ...
        boolean needsRefresh = SharedPreferencesUtil.getInt("last_wbi", 0) != curr
                || mixin_key.length() != 32;
        if (needsRefresh) {
            Logu.d("检查WBI");
            try {
                String rawKey = ConfInfoApi.getWBIRawKey();
                if (rawKey.length() < 64) throw new JSONException("WBI 原始密钥长度异常");
                mixin_key = ConfInfoApi.getWBIMixinKey(rawKey);
                // 只有完整取到密钥后才记录日期，避免一次弱网失败污染整天缓存。
                ...
                Logu.e("WBI", "刷新失败，尝试旧密钥或兼容接口: " + error);
                if (mixin_key.length() != 32) return url_query;
            }
        }
