        ...
        TerminalContext.getInstance().getVideoInfoByAidOrBvId(playerData.aid, "").observe(this, result -> result.onSuccess((videoInfo -> {
            ...
            if (intent.getIntExtra("download", 0) == 1) {    //下载模式
                adapter.setOnItemClickListener(position -> {
                    ...
                    try {
                        downPath = VideoStorageUtil.currentTaskDirectory(this, videoInfo.title,
                                videoInfo.pagenames.get(position), false);
                    } catch (Exception ignored) {
                    }
                    if (downPath != null && downPath.exists()) {
                        MsgUtil.showMsg(downPath.find(".DOWNLOADING") != null
                                ? "已在下载队列" : "已下载完成");
                    } else {
                        startActivity(
                                new Intent()
