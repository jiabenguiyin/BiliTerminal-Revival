        ...
        final List<SettingSection> sectionList = new ArrayList<>() {
            {
                ...
                add(new SettingSection("switch", "禁用返回键", "back_disable", getString(R.string.desc_back_disable),
                        "false"));
                add(new SettingSection("switch", "主页防误滑退出", SharedPreferencesUtil.HOME_SWIPE_DISMISS_DISABLE,
                        "开启后，推荐、热门、动态等主页不会被系统滑动手势直接关闭；详情页仍可滑动返回。重进主页后生效。", "true"));
                ...
                add(new SettingSection("switch", "加载渐入渐出动画", SharedPreferencesUtil.LOAD_TRANSITION,
                        getString(R.string.desc_load_transition),
                        Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ? "true" : "false"));
