        ...
        asyncInflate(R.layout.activity_setting_about, (layoutView, resId) -> {
            ...
            findViewById(R.id.author_words).setOnClickListener(view -> {
                ...
                if (eggClick_authorWords == 7) {
                    ...
                    MsgUtil.showText("维护日志", getString(R.string.egg_about_author_words));
                }
            });
            ...
            findViewById(R.id.toUncle).setOnClickListener(view -> {
                ...
                if (eggClick_toUncle == 7) {
                    eggClick_toUncle = 0;
                    MsgUtil.showText("给未来维护者", getString(R.string.egg_about_to_uncle));
                }
            });

            findViewById(R.id.icon_license_list).setOnClickListener(v -> {
                StringBuilder str = new StringBuilder(getString(R.string.open_source_notice));
                str.append("\n\n").append(getString(R.string.desc_icon_license));
                ...
                MsgUtil.showText("开源、原作者与许可", str.toString());
            });

            findViewById(R.id.gpl_license_full).setOnClickListener(v -> {
                try {
                    String license = new String(
                            NetWorkUtil.readStream(getResources().openRawResource(R.raw.gpl_3_0)),
                            "UTF-8"
                    );
                    MsgUtil.showText("GNU GPL-3.0", license);
                } catch (Exception error) {
                    MsgUtil.err("读取许可证", error);
                }
            });
